...
module Paperclip
  module Validators
    class VideoAttachmentDurationValidator < ActiveModel::Validations::NumericalityValidator
      ...
      def self.helper_method_name
        :validates_video_attachment_duration
      end

      def validate_each(record, attr_name, value)
        base_attr_name = attr_name
        attr_name = "#{attr_name}_file_duration".to_sym
        value = FFMPEG::Movie.new(value.queued_for_write[:original].path).duration
    ...
    module HelperMethods
      ...
      def validates_video_attachment_duration(*attr_names)
        options = _merge_attributes(attr_names)
        validates_with VideoAttachmentDurationValidator, options.dup
        validate_before_processing VideoAttachmentDurationValidator, options.dup
      end
    end
  end
end
